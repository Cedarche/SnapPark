...
import { officeYearCombinator } from "../Billing/Functions/Combinator";

var weekNames = ["", "M", "T", "W", "T", "F", ""];
var panelAttributes = { rx: 3, ry: 3 };

// var panelColors = ["#EEEEEE", "#8fff73", "#01ced2"];
var panelColors = ["#EEEEEE", "#BFF7B1", "#8fff73", "#48E7A3", "#48E7A3"];
...
const Stats: React.FC<Props> = ({ officeData, usage }) => {
  ...
  useEffect(() => {
    if (officeData) {
      const spotsList = officeData.parkingSpots;
      if (!spotsList || spotsList?.length < 1) {
        console.log(spotsList);
        const parkNum = 0;
        const employeeNum = 0
        const averageUtilization = 0;
        // console.log("Stats: ", { parkNum, employeeNum, averageUtilization });

        const stats: Stats[] = [
          // Ensure the 'stats' array conforms to 'Stats[]'
          { name: "Number of carparks", value: parkNum },
          { name: "Number of employees", value: employeeNum },
          { name: "Average utilisation", value: averageUtilization, unit: "%" }, // 'unit' is now allowed to be a string
        ];

        setStatistics(stats);
        return;
      } else if (spotsList || spotsList.length >= 1) {
        const parkNum = spotsList?.length;
        const employeeNum = officeData.notificationList.length;
        const averageUtilization = Number(
          (
            spotsList.reduce((acc, spot) => acc + spot.utilisation, 0) /
            spotsList.length
          ).toFixed(2),
        );
        // console.log("Stats: ", { parkNum, employeeNum, averageUtilization });
    ...
    const yearUsage = officeYearCombinator(usage, 2024, officeData?.id);
    if (yearUsage) {
      const newFormattedValues = yearUsage.reduce(
  ...
  return (
    <div className="flex flex-col  h-full">
      <div className="flex flex-col flex-grow border-b p-5 lg:pl-5 lg:pr-5 lg:pb-5 mt-2">
        <div className="flex justify-between items-center pl-3 pb-4 lg:px-1">
          <span className="text-2xl font-semibold  tracking-normal text-grey">
            Usage by Day
          </span>
          <span className="text-md font-normal  text-gray-500 tracking-wider">
            {currentDate}
          </span>{" "}
          {/* Display current date */}
        </div>
        <div className="flex-grow px-3 pb-5 md:px-0 mt-0">
          <Calendar
          ...
          {/* <div className="flex items-center justify-end w-full pr-2 text-xs">
            Less
        ...
        <div className="flex items-center justify-end w-full pr-2 text-xs">
          <Link to="/dashboard/billing">
            <span
              className="inline-flex mb-2 mt-2 lg:mb-0 lg:mt-0 items-center gap-x-1.5 rounded-md bg-blue-50 px-3 py-1 text-sm 
      ...
      <div className="mx-auto max-w-7xl">
        <div className="grid grid-cols-1 gap-px bg-white/5 sm:grid-cols-3 lg:grid-cols-3">
          {stats.map((stat: any) => (
            <div
              key={stat.name}
              className="bg-gray-50  py-4 px-4 lg:px-4 overflow-hidden"
            >
              <p className="text-[10px] 2xl:text-xs font-medium leading-6 text-gray-500 ">
                {stat.name}
              </p>
              <p className="mt-0 2xl:mt-2 flex items-baseline gap-x-2">
                <span className="text-4xl md:text-xl  xl:text-3xl 3xl:text-4xl font-semibold tracking-tight text-grey">
